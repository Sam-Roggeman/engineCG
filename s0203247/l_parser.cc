...
std::string LParser::LSystem2D::get_finalstring(std::string current, int iterationnr, int totaliterations ) {
    if (iterationnr == totaliterations){return current;}
    else{
        std::string next = "";
        for (char c: current){
            if (c =='+' || c=='-' || c == '(' || c == ')') next += c;
            else next += this->get_replacement(c);
        }
        return get_finalstring(next, iterationnr+1,totaliterations);
    }
}

std::string LParser::LSystem2D::get_finalstring() {
    return get_finalstring(this->get_initiator(), 0,this->get_nr_iterations());
}
...
std::string LParser::LSystem3D::get_finalstring(std::string current, int iterationnr, int totaliterations ) {
    if (iterationnr == totaliterations) { return current; }
    else {
        std::string next = "";
        for (char c: current) {
            ...
            || c == '^' || c == '&' || c == '\\' || c == '/' || c == '|')
                next += c;
            else if (this->get_alphabet().find(c) != this->get_alphabet().end()){
                next += this->get_replacement(c);
            }
        }
        return get_finalstring(next, iterationnr + 1, totaliterations);
    }
...
std::string LParser::LSystem3D::get_finalstring() {
    return get_finalstring(this->get_initiator(), 0,this->get_nr_iterations());
}

bool LParser::LSystem3D::cInAlphabet(const char &c) {
