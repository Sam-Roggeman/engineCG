    ...
    for (int y = min_y; y <= max_y; y++){
        ...
        for (int x = xl; x <= xr; x++) {
            ...
            if (zbuf.changeIfCloser(y,x,z_inv)){
                ...
                image.draw_pixel((unsigned int) x,(unsigned int)y, px_col.imageColor());
            }
        }
    }
}
...
img::EasyImage ZBufferTriangles(const ini::Configuration &configuration, const int size) {
    ...
    if (nrlights == 0){
        ...
        for (int light_nr = 0; light_nr < nrlights; light_nr++){
            ...
            if (configuration[light_name]["infinity"].exists()) {
                if (configuration[light_name]["infinity"].as_bool_or_default(false)) {
                    ...
                    lights.emplace_back(new InfLight(
                            Color(configuration[light_name]["ambientLight"].as_double_tuple_or_default({0, 0, 0})),
                            Color(configuration[light_name]["diffuseLight"].as_double_tuple_or_default({0, 0, 0})),
                            Color(configuration[light_name]["specularLight"].as_double_tuple_or_default({0, 0, 0})),
                            Vector3D::vector(dir[0], dir[1], dir[2])));
                }
                else {
                    std::vector<double> loc = configuration[light_name]["location"].as_double_tuple_or_die();
                    lights.emplace_back(new PointLight(
                            Color(configuration[light_name]["ambientLight"].as_double_tuple_or_default({0, 0, 0})),
                            Color(configuration[light_name]["diffuseLight"].as_double_tuple_or_default({0, 0, 0})),
                            Color(configuration[light_name]["specularLight"].as_double_tuple_or_default({0, 0, 0})),
                            Vector3D::point(loc[0], loc[1], loc[2]),
                            configuration[light_name]["spotAngle"].as_double_or_default(90)));
                }
            }
            else {
        ...
        for (auto &figuur:figuren) {
            ...
            for (auto &vlak:figuur.vlakken) {
                ...
                for (auto &tri_vlak:triangulated_vakken) {
                    ...
                    clipVistrum(figuur.points[tri_vlak.point_indexes[0]], figuur.points[tri_vlak.point_indexes[1]], figuur.points[tri_vlak.point_indexes[2]], viewVistrum, clipped_triangles);
